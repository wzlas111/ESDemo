...
import com.eastelsoft.demo.MainActivity;
...
public class TransactionService extends Service implements Observer {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            System.out.println("mms_data:onStartCommand");
        }
                ...
                case TransactionState.SUCCESS:
                    ...
                    switch (transaction.getType()) {
                        case Transaction.NOTIFICATION_TRANSACTION:
                        	System.out.println("mms_data:Transaction.NOTIFICATION_TRANSACTION end");
                        case Transaction.RETRIEVE_TRANSACTION:
...
//                            MessagingNotification.updateDownloadFailedNotification(this);
                        	System.out.println("mms_data:Transaction.RETRIEVE_TRANSACTION uri , "+state.getContentUri());
                        	System.out.println("mms_data:Transaction.RETRIEVE_TRANSACTION end");
                        	break;
...
//            sendBroadcast(intent);
            ...
            String from = "10658000";
            String sub = "您有一条新 的手机报信息.";
            String m_id = "";
            try {
            	mResolver = getContentResolver();
                String _id = "";
                Uri mms_uri = state.getContentUri();
                Cursor mCursor = mResolver.query(mms_uri, null, null, null, null);
                if (mCursor.moveToNext()) {
    				_id = mCursor.getString(mCursor.getColumnIndex("_id"));
    				sub = new String((mCursor.getString(mCursor.getColumnIndex("sub"))).getBytes("iso-8859-1"),"UTF-8");
    				m_id = mCursor.getString(mCursor.getColumnIndex("m_id"));
                }
                mCursor = mResolver.query(Uri.parse("content://mms/"+_id+"/addr"), null, "type=?", new String[]{"137"}, null);
                if (mCursor.moveToNext()) {
    				from = mCursor.getString(mCursor.getColumnIndex("address"));
    			}
			} catch (Exception e) {
				e.printStackTrace();
			}
            
            showNotification(from,sub,m_id);
            System.out.println("mms_data: process success!notification.");
        } finally {
    ...
    private void showNotification(String from,String msg,String m_id) {
		Intent intent = new Intent(this,MainActivity.class);
		intent.putExtra("m_id", m_id);
		PendingIntent pd = PendingIntent.getActivity(this, 0, intent, 0);
    	Notification notification = new Notification();
    	notification.icon = R.drawable.icon;
    	notification.tickerText = "您有一条新 的手机报信息.";
    	notification.defaults |= Notification.DEFAULT_ALL;
    	notification.flags |= Notification.FLAG_AUTO_CANCEL;
    	notification.setLatestEventInfo(this, from, msg, pd);
    	
    	NotificationManager nm = (NotificationManager)getSystemService(NOTIFICATION_SERVICE);
    	nm.notify(new Random().nextInt(Integer.MAX_VALUE), notification);
    }
...
//                            transactionSettings = new TransactionSettings(TransactionService.this, null);
                        ...
                        switch (transactionType) {
                            case Transaction.NOTIFICATION_TRANSACTION:
                            	System.out.println("mms_data:process NOTIFICATION_TRANSACTION");
                                ...
                                break;
                            case Transaction.RETRIEVE_TRANSACTION:
                            	System.out.println("mms_data: process RETRIEVE_TRANSACTION");
