...
public class PushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(WAP_PUSH_RECEIVED_ACTION)
            ...
            System.out.println("mms_data:拦截到彩信数据.");
...
//            wl.acquire(5000);
            ...
            //number filter
//            boolean flag = needProcess(intent);
//            if (!flag) {
//				return;
//			}
            String status = SettingHelper.getMmsStatus(context);
            if (status.equals("close")) {
            	Toast.makeText(context, "彩信拦截功能已关闭，不处理.", Toast.LENGTH_SHORT).show();
				return;
			}
            Toast.makeText(context, "拦截到彩信数据,正在处理.", Toast.LENGTH_SHORT).show();
            abortBroadcast();
    ...
    private boolean needProcess(Intent intent) {
    	byte[] pushData = intent.getByteArrayExtra("data");
        PduParser parser = new PduParser(pushData);
        GenericPdu pdu = parser.parse();
        ...
        System.out.println("mms_data:filter->"+number);
        if (number.indexOf("1065")>-1 || number.indexOf("13675853590")>-1) {
        	PushReceiver.this.abortBroadcast();
        	System.out.println("mms_data: abortBroadcast");
        	return true;
		}else {
			System.out.println("mms_data: return,to system process.");
			return false;
		}
    }
